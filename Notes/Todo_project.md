...
- **Doc Shop** (intention vs code actuel, linkage inventaire) : `Notes/Ui/DOC_feature_shop.md`.
...
- [ ] **Session suivante — test persistance gameplay JSON (priorité)** : valider la nouvelle implémentation ferme (`FarmSaveService`) sur scénario complet: poser -> quitter -> relancer -> vérifier plantes/stades/occupations ; tester récolte/arrache puis relance, et noter les ajustements à faire.
- [x] **Laitue — un seul `PlantDefinition` (Data/Ferme)** : objectif atteint — **`Assets/Data/Ferme/Laitue.asset`** comme source unique ; plus de doublon SO sous l’ancien chemin `Scripts/Data`. Si une régression réapparaît, repasser les prefabs (`LaitueObj`, `SeedSelectionUI`) et les scènes dans l’Inspector.
- [ ] **Session suivante — audit doc globale projet** : faire un audit complet des docs/notes pour aligner avec le code actuel (retirer l’obsolète `Inventaire.unity` runtime, harmoniser architecture Scene/UI, clarifier les flux actifs), puis proposer un plan de consolidation.
- [ ] **Session suivante — passe commentaires FR sur tout le projet** : faire une passe de lisibilité sur **l’ensemble des scripts** (`Assets/Scripts/**`) pour ajouter/mettre à jour des commentaires en français là où la logique n’est pas immédiatement claire ; harmoniser le style de commentaires sans surcharger le code.
...
- [ ] **Architecture services cloud (inventaire + market)** : appliquer la spec **`Notes/Ui/SPEC_services_inventory_market_cloud.md`** (interfaces `IInventoryService`/`IMarketService`, UI vue-only, états async `Loading/Ready/Error/Stale/Syncing`, cache local + retry/refresh, validation serveur autoritaire CloudScript/PHP/MySQL).
- [ ] **Prérequis cloud — découplage complet inventaire/gameplay** : retirer les dépendances directes à `PlayerInventory.Instance` dans le gameplay et passer par `IInventoryService` (ou provider). **À faire avant** la sync cloud, ou au plus tard dans la toute première phase d'implémentation cloud. Réf : `Notes/Ui/SPEC_services_inventory_market_cloud.md`.
