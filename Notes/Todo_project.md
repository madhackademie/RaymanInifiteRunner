...
- [ ] **Récolte Mature / Graines (Bezi) — câblage + SO** : finaliser le **câblage scène et Inspector** du système de récolte **mature vs graines** issu ou complété via **Bezi** (`HarvestPanelUI`, `PlantHarvestInteractor`, `BiofiltreManager`, prefab plante / panel). **Créer ou compléter les ScriptableObjects** : `PlantDefinition` (stades / footprint / durées), **`ItemDefinition`** pour chaque récolte visée (ex. feuille vs graine), entrées dans **`ItemDatabase`** ; vérifier les **définitions** alignées avec les stades `Mature` et `Seedling` dans `PlantGrow`. **Prévoir une première session « lecture seule »** : parcourir le flux grille → popup → `ConfirmHarvest` → `PlayerInventory.TryAdd` jusqu’à bien **comprendre le fonctionnement** global (carte : `Notes/Farm/SYSTEMES_carte_mentale.md` Zoom D, `Notes/Codebase_etat_reference.md`).
- [ ] **Inventaire récolte — finaliser et câbler** *(sortie de session 2026-04-11)* : boucler le flux complet **récolte → inventaire** en scène : références Inspector (`HarvestPanelUI`, injection panel dans `PlantHarvestInteractor` / `BiofiltreManager`), `PlayerInventory`, `ItemDatabase` + `ItemDefinition` pour l’item récolté, `InventoryFeedbackUI`, prefab panel / slots si besoin ; scène **`FirstLvl`** (ou niveau de test). Vérifier : clic cellule **mature** → ouverture panel → **Récolter** → `TryAdd`, UI à jour, cas **inventaire plein**. Schéma : Zoom D dans `Notes/Farm/SYSTEMES_carte_mentale.md`.
